...
const TABS = [
  ...
  { key: 'examen', label: 'Examen Estético', icon: EyeIcon },
...
const EXPECTATIVAS_LABEL: Record<string, string> = {
  realistas: 'Realistas',
  parcialmente_realistas: 'Parcialmente realistas',
  no_realistas: 'No realistas',
};

const OPTIMO_LABEL: Record<string, string> = {
  si: 'Sí',
  necesita_examenes: 'Necesita exámenes previos',
  no: 'No',
};

function buildAnamnesisSummary(patient: Patient): string {
  const age = calculateAge(patient.birthDate);
  const parts: string[] = [];
  const genderWord = patient.gender === 'femenino' ? 'Paciente femenina' : patient.gender === 'masculino' ? 'Paciente masculino' : 'Paciente';
  parts.push(`${genderWord}${age !== null ? ` de ${age} años` : ''}.`);

  if (patient.chronicDiseases) parts.push(`${patient.chronicDiseases}.`);
  else parts.push('Sin antecedentes mórbidos relevantes registrados.');

  if (patient.allergies.length > 0 || patient.allergyNotes) {
    parts.push(`Alergias: ${patient.allergyNotes || patient.allergies.join(', ')}.`);
  } else {
    parts.push('Niega alergias medicamentosas, cosméticas o al látex.');
  }

  if (patient.dentalHistory) parts.push(`Antecedentes: ${patient.dentalHistory}.`);
  if (patient.currentMedications) parts.push(`Medicamentos actuales: ${patient.currentMedications}.`);
  if (patient.motivoConsulta) parts.push(`Consulta por: ${patient.motivoConsulta}`);
