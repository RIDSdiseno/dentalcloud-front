...
export function RxOrderDetailModal({ orderId, onClose }: { orderId: number; onClose: () => void }) {
  ...
  return (
    <Modal title={`Orden Rx N° ${orderId}`} onClose={onClose} maxWidth="max-w-3xl">
      {isLoading ? (
        <p className="py-8 text-center text-sm text-slate-400 dark:text-slate-500">Cargando orden...</p>
      ) : !order ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Diagnóstico</label>
              <input
                value={diagnostico}
                onChange={(e) => setDiagnostico(e.target.value)}
                disabled={!isEditable}
                className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-brand-500 focus:ring-3 focus:ring-brand-500/15 disabled:bg-slate-50 disabled:text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:disabled:bg-slate-800/60 dark:disabled:text-slate-500"
              />
            </div>
            <div>
          ...
          <div>
            <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Observaciones</label>
            <textarea
              value={observaciones}
          ...
          <div className="flex flex-col gap-3">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">Exámenes</h3>
            {order.examenes.map((exam) => (
              <div key={exam.id} className="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
                <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{exam.descripcion}</p>
                ...
                <div className="flex flex-col gap-1.5">
                  {exam.archivos.map((file) => (
                    <div key={file.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-1.5 text-sm dark:bg-slate-800/60">
                  ...
                  {exam.archivos.length === 0 && <p className="text-xs text-slate-400 dark:text-slate-500">Sin archivos adjuntos.</p>}
                </div>
                ...
                {isEditable && (
                  <div className="mt-2 flex items-center gap-2">
                    <input
                      ...
                      type="file"
                      multiple
                      className="flex-1 text-xs dark:text-slate-300"
                    />
                    <button
                      type="button"
                      onClick={() => handleUpload(exam.id, exam.id_tipo_examen)}
                      disabled={uploadingExamId === exam.id}
                      className="flex items-center gap-1 rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
                    >
