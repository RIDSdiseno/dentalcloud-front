  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-100">Formatos</h2>
        <div className="relative">
          ...
          {menuOpen && (
            <div className="absolute right-0 z-10 mt-1 w-48 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg dark:border-slate-700 dark:bg-slate-900">
      ...
      {loading ? (
        ...
        <p className="text-sm text-slate-400 dark:text-slate-500">Aún no hay formatos creados.</p>
      ) : (
        <table className="w-full text-sm">
          ...
          <tbody>
            {formats.map((format) => (
              <tr key={format.id} className="border-b border-slate-50 dark:border-slate-800">
                <td className="py-2.5">
                  {renamingId === format.id ? (
                    <input
                      ...
                      className="w-full rounded border border-slate-300 px-2 py-1 text-sm outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500"
                    />
                  ) : (
                    <span className="font-medium text-slate-700 dark:text-slate-200">{format.name}</span>
                  )}
                </td>
                <td className="py-2.5 text-slate-500 dark:text-slate-400">{FORMAT_TYPE_LABEL[format.type]}</td>
                <td className="py-2.5 text-slate-500 dark:text-slate-400">{new Date(format.updatedAt).toLocaleDateString('es-CL')}</td>
                <td className="py-2.5">
                  <div className="flex items-center justify-end gap-1.5">
                    <button
                      type="button"
                      onClick={() => setPreviewFormat(format)}
                      className="rounded-lg px-2 py-1 text-xs font-semibold text-brand-700 hover:bg-brand-50 dark:text-brand-400 dark:hover:bg-brand-500/10"
                    >
                      Vista previa
                    </button>
                    <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-6" onClick={() => setPreviewFormat(null)}>
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">{previewFormat.name}</h3>
              <button type="button" onClick={() => setPreviewFormat(null)} className="text-xs font-semibold text-slate-500 underline dark:text-slate-400">
                Cerrar
              </button>
            </div>
            <textarea
              ...
              className="w-full rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700 outline-none focus:border-brand-500 focus:bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:bg-slate-900"
            />
          </div>
        </div>
      )}
